...
def find_beautiful_languages(languages)
  ...
  languages.select { |_name, data| data if data[:is_beautiful?] }
end
...
def find_language_facts(languages, language_name, _fact_index = 0)
  ...
  array_facts = languages.dig(language_name, :facts)
  return nil if array_facts.nil?

  array_facts[_fact_index]
end
